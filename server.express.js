...
async function start() { // connect first, then start server
  try {
...
{ // age from "year" field (if age in future can go negativee)
...
app.post('/login', async (req, res) => {
  ...
  if (!existing) {
    await Users.insertOne({ username, password }); // minimal (plaintext for assignment)
...
// ---- pages ----------------
app.get('/', (req, res) => {
  if (req.signedCookies.user) return res.redirect('/app');
